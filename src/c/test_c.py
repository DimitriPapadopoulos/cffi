...
def _test_wchar_variant(typename):
    ...
    assert str(cast(BWChar, 0x45)) == f"<cdata '{typename}' 'E'>"
    assert str(cast(BWChar, 0x1234)) == f"<cdata '{typename}' '\u1234'>"
    if not _hacked_pypy_uni4():
        if wchar4:
            x = cast(BWChar, 0x12345)
            assert str(x) == f"<cdata '{typename}' '\U00012345'>"
            assert int(x) == 0x12345
        else:
            x = cast(BWChar, 0x18345)
            assert str(x) == f"<cdata '{typename}' '\u8345'>"
            assert int(x) == 0x8345
    #
    ...
    #
    w = cast(BWChar, 'a')
    assert repr(w) == f"<cdata '{typename}' 'a'>"
    assert str(w) == repr(w)
    assert string(w) == 'a'
    assert int(w) == ord('a')
    w = cast(BWChar, 0x1234)
    assert repr(w) == f"<cdata '{typename}' '\u1234'>"
    assert str(w) == repr(w)
    assert string(w) == '\u1234'
    assert int(w) == 0x1234
    w = cast(BWChar, '\u8234')
    assert repr(w) == f"<cdata '{typename}' '\u8234'>"
    assert str(w) == repr(w)
    assert string(w) == '\u8234'
    assert int(w) == 0x8234
    w = cast(BInt, '\u1234')
    ...
    if wchar4 and not _hacked_pypy_uni4():
        ...
        assert repr(w) == f"<cdata '{typename}' '\U00012345'>"
        assert str(w) == repr(w)
